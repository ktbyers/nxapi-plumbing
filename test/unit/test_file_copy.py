...
class FileCopyTestCase(unittest.TestCase):
    ...
    def test_file_already_exists(self, mock_local_md5):
        ...
        self.device.show.return_value = {
            "file_content_md5sum": "b211e79fbaede5859ed2192b0fc5f1d5\n"
        }

        result = self.fc.already_transfered()

        self.assertEqual(result, True)
        self.device.show.assert_called_with(
            "show file bootflash:source_file md5sum", raw_text=False
        )
    ...
    def test_send_file(
        ...
        mock_SCP.return_value.put.assert_called_with(
            "/path/to/source_file", "bootflash:source_file"
        )
    ...
    def test_get_file(
        ...
        mock_SCP.assert_called_with(mock_ssh.get_transport.return_value)
        mock_SCP.return_value.get.assert_called_with(
            "bootflash:source_file", "/path/to/source_file"
        )
        mock_SCP.return_value.close.assert_called_with()
    ...
    @mock.patch.object(FileCopy, "local_file_exists")
    @mock.patch.object(FileCopy, "enough_space")
    def test_send_file_error_local_not_exist(
        self,
        mock_enough_space,
        mock_local_file_exists,
        mock_remote_md5,
    ...
    @mock.patch.object(FileCopy, "local_file_exists")
    @mock.patch.object(FileCopy, "enough_space")
    def test_send_file_error_not_enough_space(
        self,
        mock_enough_space,
        mock_local_file_exists,
        mock_remote_md5,
    ...
    @mock.patch.object(FileCopy, "local_file_exists")
    @mock.patch.object(FileCopy, "enough_space")
    def test_send_file_transfer_error(
        self,
        mock_enough_space,
        mock_local_file_exists,
        mock_remote_md5,
